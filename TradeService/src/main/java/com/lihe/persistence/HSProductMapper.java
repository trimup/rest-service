...
public interface HSProductMapper {
    ...
    @Select("select * from  lh_hs_product_info where  is_view=1 and ofund_type= #{ofund_type} and declare_state =1  ")
    ...
    @Select("SELECT * FROM `lh_hs_product_info` WHERE `id` = #{id}")
    public HSProductInfo findHSProductInfoById(@Param("id") Integer id);

    @Select("SELECT * FROM `lh_hs_product_info` WHERE `fund_code` = #{fund_code}")
    public HSProductInfo findHSProductInfoByFundCode(@Param("fund_code") String  fund_code);
